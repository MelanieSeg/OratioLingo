...
class _PantallaVideosState extends State<PantallaVideos> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _categorias =
              categoriasSet.toList()..sort(); // Ordenar alfabéticamente
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _aplicarFiltros() {
    ...
    if (_isSearching && _searchQuery.isNotEmpty) {
      resultados =
          resultados.where((video) {
            final String titulo =
                (video['titulo'] ?? '').toString().toLowerCase();
            final String categoria =
                (video['categoria'] ?? '').toString().toLowerCase();
            final String descripcion =
                (video['descripcion'] ?? '').toString().toLowerCase();
